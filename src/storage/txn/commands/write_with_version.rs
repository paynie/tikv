...
use txn_types::{Key, Value, KvPair};
...
impl CommandExt for RawWriteWithVersion {
    ctx!();
    tag!(raw_batch_put);
    gen_lock!(kvs: multiple(|x| &x.0));
    ...
    fn write_bytes(&self) -> usize {
        ...
        self.kvs.iter().map(|kv| cf_size + kv.0.len() + kv.1.len() + kv.0.len() + 4 + 8).sum()
    }
}
...
impl<S: Snapshot, L: LockManager> WriteCommand<S, L> for RawWriteWithVersion {
    fn process_write(self, snapshot: S, _: WriteContext<'_, L>) -> Result<WriteResult> {
        let (cf, kvs, ttls, ctx, api_version) = (self.cf, self.kvs, self.ttls, self.ctx, self.api_version);
        let mut data = vec![];
